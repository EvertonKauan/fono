...
type Props = { payment: Payment; onClose: () => void; onConfirm: (amount: number) => Promise<void> }

// Montado só enquanto o Dialog está aberto, então o estado é descartado ao fechar.
export default function EditPaymentDialog({ payment, onClose, onConfirm }: Props) {
  ...
  return (
    <Dialog open onClose={onClose} fullScreen={fullScreen} fullWidth maxWidth="xs" aria-labelledby="valor-titulo">
      <Box component="form" noValidate onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', flexGrow: 1, minHeight: 0 }}>
        <DialogTitle id="valor-titulo">Editar valor — {formatPeriod(payment.period)}</DialogTitle>
        <DialogContent>
            ...
            <MoneyField
              ...
              helperText={
                submitted && amount === null
                  ? 'Informe o valor.'
                  : `Só o valor muda; as ${payment.sessions} ${payment.sessions === 1 ? 'sessão continua' : 'sessões continuam'} calculadas pelas sessões realizadas.`
              }
            />
          </Stack>
        </DialogContent>
